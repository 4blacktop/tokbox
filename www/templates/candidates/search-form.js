this_page = function() {
    return {
        get_data: function() {
            ...
            data = $.core.get_from_api('multi/?requests={"regions": {"by_first_letter": 1}, "industries": {"as-schema": 1}, "positions": {"as-schema": 1}, "wage-ranges": {"as": "wage_ranges"}, "experience_requirements": {}, "education_requirements": {}, "availabilities": {}, "languages": {}}', $.i18n._('Unable to load course data.'))
        ...
        after_load: function() {
            ...
            $('.dropdown-menu.desired_positions li .row:not(.spaced)').each(function(i, elem) {
                $(elem).find('.item:nth-child(3n+1)').after('<div class="four-col">&nbsp;</div>').closest('.row').addClass('spaced');
            });
            $('.dropdown-menu.desired_industries li .row:not(.spaced)').each(function(i, elem) {
                $(elem).find('.item:nth-child(2n+1)').after('<div class="three-col">&nbsp;</div>').closest('.row').addClass('spaced');
            });
            $('.dropdown-menu.current_positions li .row:not(.spaced)').each(function(i, elem) {
                $(elem).find('.item:nth-child(3n+1)').after('<div class="four-col">&nbsp;</div>').closest('.row').addClass('spaced');
            });
            $('.dropdown-menu.current_industries li .row:not(.spaced)').each(function(i, elem) {
                $(elem).find('.item:nth-child(2n+1)').after('<div class="three-col">&nbsp;</div>').closest('.row').addClass('spaced');
            });
            ...
            $.each(['desired', 'current'], function(i, thing) {
                $('.dropdown-menu.'+thing+'_positions input[type="checkbox"]').off('change');
                $('.dropdown-menu.'+thing+'_positions input[type="checkbox"]').on('change', function(e) {
                    selections = {};
                    $('.dropdown-menu.'+thing+'_positions input[type="checkbox"]').each(function(i, elem) {
                        if ($(elem).is(':checked')) {
                    ...
                    $('input[name="q_'+thing+'_position"]').val(selections_str);
                });

                // Update industry input upon checkbox selection
                $('.dropdown-menu.'+thing+'_industries input[type="checkbox"]').off('change');
                $('.dropdown-menu.'+thing+'_industries input[type="checkbox"]').on('change', function(e) {
                    selections = {};
                    $('.dropdown-menu.'+thing+'_industries input[type="checkbox"]').each(function(i, elem) {
                        if ($(elem).is(':checked')) {
                    ...
                    $('input[name="q_'+thing+'_industry"]').val(selections_str);
                });
            });

            // Update 'single-tier' checkbox selections (location, experience
            $.each(['desired_locations', 'current_locations', 'experience', 'education', 'availability', 'languages', 'gender'], function(i, thing) {
        ...
        before_submit: function(data) {
            ...
            $.each(['desired_position', 'desired_industry', 'desired_location', 'desired_wage_range',
                    'current_position', 'current_industry', 'current_location', 'experience',
                    'education', 'availability', 'language', 'gender'], function(i, thing) {
                regexp = new RegExp('^'+thing+'-([0-9MF]*)$');
